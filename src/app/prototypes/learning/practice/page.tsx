...
export default async function Page({ searchParams }: { searchParams: Promise<{ age?: string }> }) {
  ...
  return <div className={styles.page}><SiteHeader activeItem="learning" /><main className={styles.index}>
    ...
    <div className={styles.activityList}>{activities.map(activity => <section className={styles.activityCard} key={activity.href}>
      <h2>{activity.title}</h2><p>{activity.description}</p><Button asChild><Link href={`/prototypes/learning/practice/${activity.href}${young ? "?age=6-9" : ""}`}>{labelText("დაწყება")}<Icon name="chevronsRight" /></Link></Button>
    </section>)}</div>
  </main></div>;
}
