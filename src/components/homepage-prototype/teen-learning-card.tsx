...
export function TeenLearningCard({ item, color }: { item: { title: string; intro: string }; color: string }) {
  return <Card asChild className={styles.card} style={{ "--card-color": color } as CSSProperties}>
    <article>
      <CardContent className={styles.copy}>
        ...
        <CardDescription className={styles.description}>{item.intro}</CardDescription>
      </CardContent>
