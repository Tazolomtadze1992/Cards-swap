...
export default function ResourcesPagePrototype() {
  ...
  useEffect(() => {
    ...
    const escape = (event: KeyboardEvent) => { if (event.key === "Escape") { setOpen(false); setPreview(null); } };
  ...
  const clear = () => { setTypes([]); setAge("all"); setOpen(false); };
    ...
    <section className={styles.catalogue} aria-labelledby="resources-count">
      <div className={styles.toolbar}>
        <div className={styles.controls}>
          <div className={styles.typeFilter} ref={filterRef}>
            <button className={styles.typeTrigger} type="button" aria-expanded={open} aria-controls="resource-type-menu" onClick={() => setOpen(value => !value)}>
              რესურსის ტიპი{types.length ? ` : ${types.length}` : ""}<ChevronDown size={18} aria-hidden="true" />
            </button>
            {open && <div className={styles.typeMenu} id="resource-type-menu">
          ...
          <div className={styles.ageFilters} role="group" aria-label="ასაკით გაფილტვრა">
            <button type="button" aria-pressed={age === "all"} onClick={() => setAge("all")}>ყველა ასაკი</button>
            {ageGroups.map(group => <button key={group} type="button" aria-pressed={age === group} onClick={() => setAge(group)}>{group}</button>)}
          </div>
      ...
      {filtered.length ? <div className={styles.grid}>
        {filtered.map(item => <article className={styles.card} key={item.id} style={{ "--card-color": item.color } as CSSProperties}>
          ...
          </button> : <Image className={styles.documentIcon} src={`/assets/resources/${item.image}`} width={92} height={80} alt="" />}
          <div className={styles.copy}><span className={styles.kind}>{item.type} · {item.age}</span><h2>{item.title}</h2><p>{item.description}</p></div>
