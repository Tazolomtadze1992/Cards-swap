...
export function ResourceCard({ item, className, onOpen }: ResourceCardProps) {
  return <article className={`${styles.card}${className ? ` ${className}` : ""}`} style={{ "--card-color": item.color } as CSSProperties}>
    ...
    {!item.video && <button className={styles.download} type="button" onClick={event => onOpen(item, event.currentTarget)}>{item.type === "დოკუმენტი" ? "დოკუმენტის გადმოწერა" : "რესურსის ნახვა"}{item.type === "დოკუმენტი" && <Image src="/assets/resources/download.svg" width={24} height={24} alt="" />}</button>}
  </article>;
}
