...
export function ServiceContactCard({ contact, onRequestCall112, onRequestSupportCall, onRequestCall }: { contact: ServiceContact; onRequestCall?: (phone: ContactNumber) => void; onRequestCall112?: () => void; onRequestSupportCall?: () => void }) {
  const titleId = useId();
  return <section className={styles.contactCard} aria-labelledby={titleId}>
    ...
    {contact.numbers.map(phone => <ContactNumberRow key={phone.tel} phone={phone} onRequestCall={onRequestCall} onRequestCall112={onRequestCall112} onRequestSupportCall={onRequestSupportCall} />)}
  </section>;
}
