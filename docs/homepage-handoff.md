...
Visual source: Figma `YqyCMj54On5xNGIHkZ1t6k`, node `1132:59613`. The hero fills at least the current viewport, with the two exported character groups anchored to its bottom edge. It grows when needed to preserve content on short screens. Hero gradient: `#1f3561` to `#3f6cc7`; FAQ uses the reverse gradient. The fixed header is transparent at the top of the hero so its gradient shows through immediately. Once scrolled, brand sections use an opaque dark blue header to keep page content from overlapping navigation. The expanded mobile menu has one continuous dark-to-light blue gradient; cream sections retain an opaque cream header.
