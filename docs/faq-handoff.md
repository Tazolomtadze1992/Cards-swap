...
Homepage navigation and the homepage `ყველას ნახვა` button open this page directly. In both card decks, the FAQ card's first click runs the existing 650ms expansion choreography. Navigation is enabled only after that transition finishes; a second click on the expanded card or its revealed FAQ action opens this page.
